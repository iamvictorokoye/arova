...
import { getAuthenticatedUser } from "../supabase/get-authenticated-user";
...
export async function getPotentialMatches(): Promise<UserProfile[]> {
  ...
  if (prefsError) {
    throw new Error(prefsError.message || "Failed to get user preferences");
  }

  const currentUserPrefs = userPrefs.preferences as UserProfile["preferences"] | null;
...
export async function likeUser(toUserId: string) {
  ...
  if (likeError) {
    throw new Error(likeError.message || "Failed to create like");
  }
  ...
  if (checkError && checkError.code !== "PGRST116") {
    throw new Error(checkError.message || "Failed to check for match");
  }
  ...
  if (existingLike) {
    ...
    if (userError) {
      throw new Error(userError.message || "Failed to fetch matched user");
    }
...
export async function getUserMatches() {
  ...
  for (const match of matches || []) {
    ...
    if (userError) {
      console.error(`Failed to fetch user ${otherUserId} for match:`, userError.message);
      continue;
    }
